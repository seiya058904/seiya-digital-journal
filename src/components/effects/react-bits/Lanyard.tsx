...
/* ── Blank pixel for unconditional useTexture ──── */

const BLANK_PIXEL =
  'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg=='

/* ── UV rects for custom image compositing ────── */
...
export default function Lanyard({
  ...
  return (
    <div className="lanyard-wrapper">
      <Canvas
        camera={{ position: position as unknown as THREE.Vector3, fov }}
        dpr={[1, 2]}
        gl={{ alpha: transparent }}
        onCreated={({ gl }) => {
          gl.setClearColor(new THREE.Color('#120F17'), transparent ? 0 : 1)
        }}
      >
...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    const baseImg = (baseMap as THREE.Texture).image as HTMLImageElement
    ...
    composite.flipY = (baseMap as THREE.Texture).flipY
  ...
  useFrame((state, delta) => {
    if (dragged) {
      ...
      card.current?.setNextKinematicTranslation({
        x: vec.x - (dragged as any).x,
        y: vec.y - (dragged as any).y,
        z: vec.z - (dragged as any).z,
      })
    }
    if (fixed.current) {
      ;[j1, j2].forEach((ref) => {
  ...
  return (
    <>
      ...
      <mesh ref={band as any}>
        {/* @ts-expect-error — meshline JSX intrinsic types are declared in global.d.ts */}
        <meshLineGeometry />
        {/* @ts-expect-error — meshline JSX intrinsic types are declared in global.d.ts */}
        <meshLineMaterial
          color="white"
