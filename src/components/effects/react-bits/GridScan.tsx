...
/* ------------------------------------------------------------------ */
/*  Fragment shader — 4‑plane tunnel (floor, ceiling, left/right wall) */
/* ------------------------------------------------------------------ */
const frag = `
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float hitIsY = 1.0;
  /* 4-plane tunnel: floor (i=0), ceiling (i=1), left wall (i=2), right wall (i=3) */
    for (int i = 0; i < 4; i++)
    {
        float isY = float(i < 2);
  ...
  /* secondary grid on the perpendicular axis */
...
function smoothDampVec2(
  ...
  if (change.length() > maxSpeed * smoothTime) change.setLength(maxSpeed * smoothTime)
...
export function GridScan({
  ...
  /* ---- uniform hot-update when props change (without full re-init) ---- */
  useEffect(() => {
    const m = materialRef.current
    if (!m) return
    const u = m.uniforms
