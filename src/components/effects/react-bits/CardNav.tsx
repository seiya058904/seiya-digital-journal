...
type CardNavProps = {
  ...
  baseColor?: string
  menuColor?: string
  onLinkClick?: (link: CardNavLink, event: ReactMouseEvent<HTMLAnchorElement>) => void
}

function CardNavComponent({
  className = '',
  items = [],
  ease = 'power3.out',
  ...
  useLayoutEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (cardNavRef.current && !cardNavRef.current.contains(e.target as Node)) {
        toggleMenu()
      }
    }
      ...
      >
        <div className="card-nav-top">
          <div
            ...
            role="button"
            aria-label={isExpanded ? 'Close menu' : 'Explore'}
          ...
          >
            ...
            <span className="hamburger-line" />
            <span className="hamburger-label">{isExpanded ? 'Close' : 'Explore'}</span>
          </div>
        </div>
              ...
              <div className="nav-card-links">
                {item.links?.map((lnk, i) => (
                  <a
                    ...
                    onClick={(event) => onLinkClick?.(lnk, event)}
                  >
...
export const CardNav = CardNavComponent
