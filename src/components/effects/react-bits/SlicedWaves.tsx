...
const SlicedWaves: FC<SlicedWavesProps> = ({
  color1 = '#FF9FFC',
  color2 = '#5227FF',
  color3 = '#B497CF',
  columns = 14,
  ...
  useEffect(() => {
    ...
    const colors = [color1, color2, color3].map(hexToRgb)
    for (const [index, color] of colors.entries()) {
      const target = u[`uColor${index + 1}`].value as Float32Array
      target[0] = color[0]
      target[1] = color[1]
      target[2] = color[2]
    }
  }, [
    color1, color2, color3, columns, rows, barThickness, speed, travel, waveSpread, rowOffset,
    softness, glow, brightness, contrast, opacity, orientation, alternate, mouseInteraction,
