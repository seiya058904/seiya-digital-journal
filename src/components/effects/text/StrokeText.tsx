...
const StrokeText = ({
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      setBox((prev) => prev && Math.abs(prev.x - next.x) < 0.5 && Math.abs(prev.width - next.width) < 0.5 && Math.abs(prev.y - next.y) < 0.5 ? prev : next)
    }
    measure()
  ...
  return (
    <span
      ...
      style={{ ...style, ['--stroke-text-height' as string]: `${Math.round(fontSize * 1.3)}px` } as CSSProperties}
      role="img"
